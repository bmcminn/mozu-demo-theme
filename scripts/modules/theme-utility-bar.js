define([
    'jquery',
    'shim!vendor/datetimepicker/jquery-simple-datetimepicker[jquery=jquery]>jquery'
  ],
  ...
  function ($) {
    ...
    $(document).ready(function () {
      var DateTimePicker = function () {
        ...
        ShowHideAction = function () {
          ...
          this.isShown = (function () {
            if (!document.cookie.split(';').filter(function (str) {
                return str.indexOf('MZ_SHOW_FUTURE_BAR') > 0;
              })) {
              return true;
            }
            else {
              var cookie = document.cookie.split(';').filter(function (str) {
                return str.indexOf('MZ_SHOW_FUTURE_BAR') > 0;
              })[0];
      ...
      DateTimePicker.prototype.init = function () {
        ...
        $(document).on('click', (function () {
          ...
          if (!this.picker.isShow()) {
            if (this.dateHasChanged.call(this)) window.location.search = this.getQueryString(this.sanitizeDate());
          }
      ...
      DateTimePicker.prototype.setDate = function (date) {
        date = date ? new Date(date).toDateString() + ', ' + new Date(date).toLocaleTimeString(navigator.language, {
          hour: '2-digit',
          minute: '2-digit'
        }) + ' CST' : 'Now';
      ...
      DateTimePicker.prototype.getQueryString = function (str) {
        ...
        if (window.location.search.indexOf('?') !== -1 && window.location.search.indexOf('mz_now') === -1) {
          queryString = window.location.search + '&mz_now=' + str;
          queryString = queryString.replace(/(&)+/g, '&');
        }
        else if (window.location.search.indexOf('?') !== -1) {
          queryString = window.location.search.replace(/mz_now.*Z/, '') + (window.location.search.indexOf('&') === -1 ? '?' : '&') + 'mz_now=' + str;
          queryString = queryString.replace(/(&)+/g, '&');
          queryString = queryString.replace(/(\?)+/g, '?');
        }
        else {
      ...
      DateTimePicker.prototype.showUrl = function () {
        this.urlBar.val(window.location.href);
        this.urlBar.on('click', function () {
          $(this).select();
        });
      };
