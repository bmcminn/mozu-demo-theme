    ...
    var CheckoutStep = Backbone.MozuModel.extend({
      ...
      FulfillmentContact = CheckoutStep.extend({
        ...
        nextDigitalOnly: function () {
          ...
          this.getOrder().apiModel.update({
            fulfillmentInfo: me.toJSON()
          }).ensure(function () {
      ...
      FulfillmentInfo = CheckoutStep.extend({
        ...
        calculateStepStatus: function () {
          ...
          if (available && available.length && _.findWhere(available, {
              shippingMethodCode: this.get('shippingMethodCode')
            })) {
        ...
        updateShippingMethod: function (code) {
          var available = this.get('availableShippingMethods'),
            newMethod = _.findWhere(available, {
              shippingMethodCode: code
            }),
            lowestValue = _.min(available, function (ob) {
              return ob.price;
            });
          ...
          _.defer((function () {
            var contacts = this.getOrder().get('customer').get('contacts'),
              isPrimaryShipping = contacts.filter(function (ob) {
                return ob.attributes.isPrimaryShippingContact;
              });
      ...
      BillingInfo = CheckoutStep.extend({
        ...
        beginApplyCredit: function () {
          ...
          if (selectedCredit) {
            var applyingCredit = _.findWhere(this.availableStoreCredits(), {
              code: selectedCredit
            });
            if (applyingCredit) {
        ...
        getDigitalCreditsToAddToCustomerAccount: function () {
          return this._cachedDigitalCredits.where({
            isEnabled: true,
            addRemainderToCustomer: true,
            isTiedToCustomer: false
          });
        },
        ...
        initialize: function () {
          ...
          _.defer(function () {
            ...
            me.set('savedPaymentMethodId', savedCardId, {
              silent: true
            });
          ...
          this.on('change:isSameBillingShippingAddress', function (model, wellIsIt) {
            if (wellIsIt) {
              billingContact.set(this.parent.get('fulfillmentInfo').get('fulfillmentContact').toJSON(), {
                silent: true
              });
            }
          });
        ...
        calculateStepStatus: function () {
          var fulfillmentComplete = this.parent.get('fulfillmentInfo').stepStatus() === 'complete',
            ...
            paymentTypeIsCard = activePayments && !!_.findWhere(activePayments, {
              paymentType: 'CreditCard'
            }),
            paymentTypeIsPayPal = activePayments && !!_.findWhere(activePayments, {
    ...
    var CheckoutPage = Backbone.MozuModel.extend({
      ...
      saveCustomerCard: function () {
        ...
        if (!billingContact.id || billingContact.id === -1 || billingContact.id === 1 || billingContact.id === 'new') {
          billingContact.types = !isSameBillingShippingAddress ? [{
            name: 'Billing',
